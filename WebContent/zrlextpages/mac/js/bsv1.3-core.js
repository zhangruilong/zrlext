...
Sidebar=function(me){
	...
	return me ={
		 ...
		 createScreenTool:function(){//声音设置
		 ...
		 bindTheme:function(){
			 ...
			 $("a",themsSetting).click(function(){
					...
					var h = $(window).height();
					var w = $(window).width();
					$("#zoomWallpaper").attr("src","images/bg/theme_"+src+".jpg").width(w).height(h);
...
ElsePanel = function(me){
	var mome ="<div id=\"mome\"><div class=\"infotitl\">&nbsp;</div><p>&nbsp替换掉所有的无关图片！</br></br></p></div>";
	var inform="<div id=\"inform\"><div class=\"infotitl\">&nbsp;</div><p><a href=\"#\">&nbsp已完成对网页的基础搭建！</a></p></div>";
	...
	return me = {
		...
		openApp:function(id,title,url,icon,jsonSonMenu,width,height){//打开应用窗体
			...
			if(taskItem.length==1){
				...
				var len = BottomBar.getItemNum();// 任务图标集合 大于7不让添加
