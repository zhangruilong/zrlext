...
layui.define(function(exports){
  ...
  Dates.viewDate = function(Y, M, D){
    ...
    Dates.each(as.tds, function(i, elem){
      ...
      Dates.options.festivaljson && Dates.festivaljson(elem,YY+'-'+MM + '-' + DD,Dates.options.bwjson);
  ...
  //备忘录添加
  Dates.festivaljson = function(td, ymd ,json){
	  /*for(var bwi=0; bwi<json.length;bwi++){
		if(ymd==json[bwi].start){
			Dates.addClass(td,'bw-bt');
			td.setAttribute('data-id',json[bwi].id);
			td.setAttribute('title',json[bwi].name);
		};   
	  }*/
	  layui.each(json,function(index,itme){
		if(ymd==itme.starttime){
			Dates.addClass(td,'bw-bt');
			td.setAttribute('data-id',itme.id);
			td.setAttribute('title',itme.title);
		};     
	  });
  };
